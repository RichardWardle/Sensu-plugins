...
### check-ssl-cert-expire.py
I have some certificates that live on servers in my DMZ on a mix of windows and unix machines. I also need to be able to monitor external certificates on other servers to make sure they dont expire that are not under my control e.g. I provided a certificate to an external provider since they host a site for me under my domain that I have a CNAME record to. Add your domain to the hostname array of dictionaries, you need to provide the DNS name, port it listens on. There is an error and a warn key there which takes an integer which represents days. A warning is thrown if it expires in less than specified date and same for error. All times are calculated in GMT and its the machines job to ensure its own time is set correctly as thats what I use. I do not do any host verification checking e.g. if you have something that sits inline and does SSL inspection or terminates my connection upstream you may hit issues
